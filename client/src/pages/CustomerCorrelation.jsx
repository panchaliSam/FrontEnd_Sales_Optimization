...
const ECommerce = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const [predictResponse, paymentMethodResponse, monthlyCustomersResponse] =
        await Promise.all([
          handlePredict(formData),
          handlePaymentMethodData(formData),
          handleMonthlyCustomersData(formData)
        ]);

      // If successful, set the forecast data from response
      setForecastData(predictResponse.data);
      ...
      const monthlyCustomerResponseData = monthlyCustomersResponse.data.reduce(
        (acc, item) => {
          ...
          if (monthIndex !== -1) {
            existingYear.data[monthIndex] = item.Unique_Customers;
          }
      ...
      setError("Error uploading the file or fetching predictions.");
    }
  };
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
            <CardDataStats
              ...
              total={forecastData.customer_count}
            >
              <svg
            ...
            <CardDataStats
              ...
              total={forecastData.average_rating.toFixed(1)}
            >
              <svg
                className="fill-primary dark:fill-white"
                width="22"
